...
module.exports = (db) => {
  ...
  router.post('/start', authMiddleware, (req, res) => {
    ...
    db.beginTransaction(err => {
      ...
      db.query('SELECT * FROM members WHERE id = ?', [member_id], (err, memberResults) => {
        if (err) {
          return db.rollback(() => {
        ...
        // 检查会员是否有未结束的上机记录
        db.query('SELECT * FROM usage_records WHERE member_id = ? AND end_time IS NULL', [member_id], (err, activeRecords) => {
          if (err) {
            return db.rollback(() => {
              res.status(500).json({ error: err.message, member_id: req.body.member_id });
            });
          ...
          // 2. 检查机位是否存在且空闲
          db.query('SELECT * FROM machines WHERE id = ?', [machine_id], (err, machineResults) => {
            if (err) {
              return db.rollback(() => {
                res.status(500).json({ error: err.message, member_id: req.body.member_id });
              });
    ...
    db.beginTransaction(err => {
      ...
      db.query(
        ...
        (err, results) => {
          ...
          // 查询机位单价
          db.query('SELECT hourly_rate FROM machines WHERE id = ?', [record.machine_id], (err, machineResults) => {
            if (err) {
              return db.rollback(() => {
                res.status(500).json({ error: err.message });
              });
          ...
          // 3. 检查会员余额
          db.query('SELECT * FROM members WHERE id = ?', [record.member_id], (err, memberResults) => {
            if (err) {
              return db.rollback(() => {
                res.status(500).json({ error: err.message });
              });
